...
// Compression middleware to reduce response sizes over the tunnel
app.use(compression());

// Simple timing middleware to log slow requests (>= 500ms)
app.use((req, res, next) => {
    const start = Date.now();
    res.on('finish', () => {
        const ms = Date.now() - start;
        if (ms >= 500) {
            console.log(`SLOW_REQUEST: ${req.method} ${req.originalUrl} took ${ms}ms`);
        }
    });
    next();
});
...
// Use express-session for local development to avoid any cookie-session edge cases
// while keeping cookie-session for production (cloud) deployments.
let sessionMiddleware = cookieSessionMiddleware;
if (process.env.NODE_ENV === 'production') {
    ...
    const expressSessionMiddleware = session({
        ...
        cookie: {
            maxAge: cookieSessionOptions.maxAge,
            secure: false,
            sameSite: cookieSessionOptions.sameSite
        }
    });
    app.use(expressSessionMiddleware);
    sessionMiddleware = expressSessionMiddleware;
    // For local dev, enable cookie parsing to read JWT fallback cookie
    app.use(cookieParser());
}
...
const requireAuth = (req, res, next) => {
    ...
    // Local JWT fallback: if no session, check for local_jwt cookie (only for non-production/local)
    try {
        const token = req.cookies && req.cookies.local_jwt;
        if (token) {
            const payload = jwt.verify(token, SESSION_SECRET);
            if (payload && payload.userId) {
                req.userId = payload.userId;
                // populate session if possible
                try { if (req.session) req.session.userId = payload.userId; } catch (e) { }
                return next();
            }
        }
    } catch (e) {
        console.warn('local_jwt verification failed', e && e.message ? e.message : e);
    }
...
// Metrics tracking for broadcastUpdate
let broadcastMetrics = { callCount: 0, totalDbTime: 0, totalEmitTime: 0, emitCount: 0, lastReset: Date.now() };
setInterval(() => {
    if (broadcastMetrics.callCount > 0) {
        const elapsed = (Date.now() - broadcastMetrics.lastReset) / 1000;
        console.log(`📊 BROADCAST METRICS: calls=${broadcastMetrics.callCount}, emits=${broadcastMetrics.emitCount}, avgDbMs=${(broadcastMetrics.totalDbTime / broadcastMetrics.callCount).toFixed(2)}, calls/min=${(broadcastMetrics.callCount / elapsed * 60).toFixed(2)}`);
    }
    broadcastMetrics = { callCount: 0, totalDbTime: 0, totalEmitTime: 0, emitCount: 0, lastReset: Date.now() };
}, 60000); // Log every minute

// Redis caching (disabled for local Docker). Configure with REDIS_URL or REDIS_HOST/REDIS_PORT in cloud deployments
let redisClient = null;
console.log('Redis: disabled for local Docker setup');
/*
try {
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (matches) {
                ...
                // Local-only: issue a signed JWT cookie fallback so local clients can authenticate reliably
                try {
                    if (process.env.NODE_ENV !== 'production') {
                        const token = jwt.sign({ userId: user.id }, SESSION_SECRET, { expiresIn: '7d' });
                        res.cookie('local_jwt', token, { httpOnly: true, maxAge: 7 * 24 * 60 * 60 * 1000, sameSite: 'lax' });
                    }
                } catch (e) { console.warn('Failed to set local_jwt cookie', e); }
                // Ensure session is saved (express-session is async); cookie will be sent on save
                try {
                    ...
                    console.warn('session save check failed', e);
                    return res.redirect('/dashboard');
                }
        } else {
...
app.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await db.query("INSERT INTO users (email, username, password) VALUES ($1, $1, $2) RETURNING id", [email, hash]);
        
        // Auto-login after successful registration
        const newUserId = result.rows[0].id;
        req.session.userId = newUserId;
        console.log(`User registered and auto-logged in: ${email} (id: ${newUserId})`);
        try {
            if (process.env.NODE_ENV !== 'production') {
                const token = jwt.sign({ userId: newUserId }, SESSION_SECRET, { expiresIn: '7d' });
                res.cookie('local_jwt', token, { httpOnly: true, maxAge: 7 * 24 * 60 * 60 * 1000, sameSite: 'lax' });
            }
        } catch (e) { console.warn('Failed to set local_jwt cookie on register', e); }
...
app.get('/logout', (req, res) => {
    try {
        ...
        try { req.session = null; } catch (e) { /* ignore */ }
    } catch (e) { /* ignore */ }

    // Clear known cookies used by auth/session by explicitly expiring them
    try { res.cookie('local_jwt', '', { httpOnly: true, expires: new Date(0), path: '/' }); } catch (e) { /* ignore */ }
    try { res.cookie('connect.sid', '', { httpOnly: true, expires: new Date(0), path: '/' }); } catch (e) { /* ignore */ }
    try { res.cookie(cookieSessionOptions.name || 'session', '', { expires: new Date(0), path: '/' }); } catch (e) { /* ignore */ }
    // Backwards-compat: clear legacy authToken if present
    try { res.cookie('authToken', '', { expires: new Date(0), path: '/' }); } catch (e) { /* ignore */ }
...
// Metrics tracking for /api/pcs endpoint
let apiPcsMetrics = { requestCount: 0, totalDbTime: 0, lastReset: Date.now() };
setInterval(() => {
    if (apiPcsMetrics.requestCount > 0) {
        const elapsed = (Date.now() - apiPcsMetrics.lastReset) / 1000;
        console.log(`📊 /api/pcs METRICS: requests=${apiPcsMetrics.requestCount}, avgDbMs=${(apiPcsMetrics.totalDbTime / apiPcsMetrics.requestCount).toFixed(2)}, req/min=${(apiPcsMetrics.requestCount / elapsed * 60).toFixed(2)}`);
    }
    apiPcsMetrics = { requestCount: 0, totalDbTime: 0, lastReset: Date.now() };
}, 60000); // Log every minute
...
app.get('/api/pcs', requireAuth, async (req, res) => {
    ...
    apiPcsMetrics.requestCount++;
    apiPcsMetrics.totalDbTime += dbTime;
    console.log(`/api/pcs returned ${rows.length} rows for userId=${userId}, dbTime=${dbTime}ms`);
        ...
        console.log(`/api/pcs completed in ${totalTime}ms (db: ${apiPcsMetrics.totalDbTime}ms)`);
        res.json(list);
    } catch (err) {
        const userId = req.userId || (req.session && req.session.userId);
...
app.get('/api/pc/:id/probe', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // Persist probe result (local batch updates for Docker)
        try {
            const payload = { pcId: pcId, status: conn.status || 'Unknown', lastStatusAt: conn.lastStatusAt || Date.now() };
            // For local Docker, always use local batch updates (no PubSub)
...
async function broadcastUpdate() {
    ...
    try {
        ...
        const dbTime = Date.now() - dbStart;
        broadcastMetrics.totalDbTime += dbTime;
        ...
        for (const ownerIdStr of Object.keys(byOwner)) {
            ...
            broadcastMetrics.totalEmitTime += (Date.now() - emitStart);
        }
        const totalTime = Date.now() - startTime;
        console.log(`broadcastUpdate completed in ${totalTime}ms (db: ${dbTime}ms, emits: ${broadcastMetrics.emitCount})`);
    } catch (e) {
