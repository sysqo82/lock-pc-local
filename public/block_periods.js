document.addEventListener('DOMContentLoaded', () => {
    ...
    // Connect to AWS WebSocket API if configured, otherwise fall back to local
    ...
    function renderPcList(pcs) {
        ...
        pcs.forEach(pc => {
            ...
            probeBtn.addEventListener('click', async () => {
                ...
                try {
                    const url = API_CONFIG ? API_CONFIG.getUrl(`api/pc/${encodeURIComponent(pc.id)}/probe`) : `/api/pc/${encodeURIComponent(pc.id)}/probe`;
    ...
    async function loadList() {
        try {
            const url = API_CONFIG ? API_CONFIG.getUrl('api/block-period') : '/api/block-period';
            const res = await fetch(url, { headers: { 'Accept': 'application/json' }, credentials: 'include' });
            if (!res.ok) {
                if (res.status === 401) {
    ...
    async function loadPcList() {
        try {
            const url = API_CONFIG ? API_CONFIG.getUrl('api/pcs') : '/api/pcs';
            const res = await fetch(url, { headers: { 'Accept': 'application/json' }, credentials: 'include' });
            if (!res.ok) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            if (editingId) {
                const url = API_CONFIG ? API_CONFIG.getUrl(`api/block-period/${editingId}`) : `/api/block-period/${editingId}`;
    ...
    listEl.addEventListener('click', async (e) => {
        ...
        if (e.target.classList.contains('btn-delete')) {
            ...
            try {
                const url = API_CONFIG ? API_CONFIG.getUrl(`api/block-period/${id}`) : `/api/block-period/${id}`;
